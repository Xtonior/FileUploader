...
public class KafkaConfig {
    ...
    @Value("${kafka.topics.s3-load-events}")
    private String s3LoadTopic;

    @Value("${kafka.topics.s3-delete-events}")
    private String s3DeleteTopic;
    ...
    public ConsumerFactory<String, Object> consumerFactory() {
        ...
        deserializer.addTrustedPackages("kz.lab.dbapp.*");

        deserializer.setTypeResolver((topic, data, headers) -> {
            if (topic.equals(s3LoadTopic) || topic.equals(s3DeleteTopic)) {
                System.out.println("DEBUG: Incoming topic is: " + topic);
                return objectMapper.getTypeFactory().constructType(MinioEvent.class);
            }
